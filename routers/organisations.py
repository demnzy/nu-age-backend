...
async def get_organisation_courses(
    ...
    is_freelance: bool | None = Query(
        None,
        description="True = only freelance-pool courses. False = only genuine org-assigned "
                    "courses. None = no filter on this field (admins get everything by default)."
    ),
    user=Depends(auth.get_current_user),
    db: Session = Depends(get_db)
):
    ...
    if user_role in ("TEACHER", "INSTRUCTOR"):
        # The per-teacher lockdown only ever applies to the freelance pool,
        # and only when the caller is actually asking for the freelance view.
        # A teacher browsing their real "My Orgs" list passes is_freelance=False
        # and gets the normal, unrestricted, shared org course list — even if
        # that org happens to be DEFAULT_ORG_ID.
        if id == DEFAULT_ORG_ID and is_freelance is True:
            teacher_id = user.id
    elif user_role in ("ADMIN", "OWNER"):
        # NOTE: assumes `user` administers org `id` (orgs are 1-admin-per-org).
        pass
    else:
    ...
    if is_freelance is not None:
        query = query.filter(models.Course.is_freelance == is_freelance)
